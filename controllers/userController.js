...
const register = (req, res, next) => {
    ...
    try {
        ...
        // const key = generateApiKey({ method: 'string', length: 8 });
        // const key = '1234abcd';
        const user = { username: body.username, password: body.password, timestamp: (body.timestamp || 0), active: (body.active || 1) };
        ...
        if (result === undefined) {
            ...
            if (!result.active) {
                ...
                if (!bcrypt.compareSync(body.password, result.password))
                    ...
                    _result.token=jwt.sign({ user: user.username }, privateKey, { algorithm: 'HS256'});
